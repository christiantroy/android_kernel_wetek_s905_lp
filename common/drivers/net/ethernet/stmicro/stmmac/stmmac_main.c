 ...
 */
extern int extern_wifi_get_status(void);
extern void extern_eth_set_led(int is_on);
extern void extern_wifi_set_led(int is_on);
static void stmmac_adjust_link(struct net_device *dev)
{
	...
	if (new_state && netif_msg_link(priv))
	{
		...
		if (phydev->link) 
		{
			printk("eth status changed, turn on led \n");
			extern_eth_set_led(1);
			if(extern_wifi_get_status() == 1)
			{
				extern_just_set_wifi_led(0);
			}
		} 
		else	
		{
			printk("eth status changed, turn off led ,extern_wifi_get_status()=%d\n",extern_wifi_get_status());
			extern_eth_set_led(0);
			if(extern_wifi_get_status() == 1)
			{
				printk("eth status changed, wifi  is on, tuner on wifi led\n");
				extern_just_set_wifi_led(1);
			}
		}
	}
