...
int wifiLedGpio = -1;
int ethLedGpio = -1;
int wifiStatus = 0;// 0=disconnect, 1=connect
...
int extern_wifi_get_status(void )
{
	return wifiStatus ;
}
EXPORT_SYMBOL(extern_wifi_get_status);
void extern_wifi_set_led(int is_on)
{
	if(wifiLedGpio > 0)
	{
		wifiStatus = is_on;
		gpio_direction_output(wifiLedGpio,is_on);
	}
	return ;
}
EXPORT_SYMBOL(extern_wifi_set_led);
void extern_just_set_wifi_led(int is_on)
{
	if(wifiLedGpio > 0)
	{
		gpio_direction_output(wifiLedGpio,is_on);
...
EXPORT_SYMBOL(extern_just_set_wifi_led);
void extern_eth_set_led(int is_on)
{
	int ret = 0;
	if(ethLedGpio > 0)
	{
		gpio_direction_output(ethLedGpio,is_on);
	}

	return ;
}
EXPORT_SYMBOL(extern_eth_set_led);
